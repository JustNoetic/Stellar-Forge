...
uniform float u_cloud_radius_au;
uniform vec3 u_cloud_pole;
uniform float u_cloud_rotation;
...
void main() {
    vec3 pole_dir = length(u_cloud_pole) > 1e-4 ? normalize(u_cloud_pole) : vec3(0.0, 1.0, 0.0);
    vec3 pos_rot = rotate_about_axis(in_position, pole_dir, u_cloud_rotation);
    vec3 norm_rot = rotate_about_axis(in_normal, pole_dir, u_cloud_rotation);

    vec3 world_pos = pos_rot * u_cloud_radius_au + u_cloud_offset;
