...
- Re-exports post-processing shaders loaded from `engine/glsl/post/`:
  - `grav_lens_starfield.frag` — Screen-space backward gravitational ray deflection pass for the GAIA starfield; uses camera ray basis vectors (`u_cam_forward`, `u_cam_right`, `u_cam_up`, `u_tan_half_fov`) to eliminate matrix singularities, deflects backward sightlines via `compute_gravitational_deflection` + Kerr frame-dragging, samples `self.starfield_fbo` with bilinear filtering, and forms continuous Einstein rings, arcs, secondary mirror images, and black hole shadow silhouettes (`gl_FragDepth = 0.999999`).
