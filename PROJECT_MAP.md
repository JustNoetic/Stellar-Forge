...
- `engine/rendering/shaders.py`: Dynamically re-exports loaded GLSL shaders:
  ...
  - `starfield.*` (`glsl/celestial/`) — GAIA catalog point sprites; per-star positions computed per frame ON THE GPU in double precision (static f8 pos0 + f4 pm/color/flux buffers, `dvec3 u_origin` + `double u_t` uniforms — no CPU packing, no per-frame VBO upload); per-eye inverse-square flux falloff `I = flux·u_flux_calib / d_eye²` in the vertex shader (`u_eye` + `u_flux_calib` uniforms; calib anchors M_G=4.67 @ 1 AU to the system sun's sprite peak `0.8333·(h·fov)²/4`, so one exposure governs planets, sun, and stars alike); FIXED PSF sprite size (peak ∝ flux — true magnitude scale; bloom grows bright stars); atmospheric refraction via `apply_atmospheric_refraction` (same parallax-weighted model + limb occlusion guard as point lights); far-plane distance clamp (eye-relative) + manual behind-camera degeneration; **vertex-stage culling** in `starfield.vert` — frustum test with sprite-extent padding and exposure-threshold culling (peak center-pixel signal < 1e-6, mirroring the frag's limiting-magnitude fade) evaluated on the APPARENT (refracted) position so refraction-lifted stars stay visible; rendered into `self.starfield_fbo` (native screen resolution) for on-screen stars and into the 6 faces of `self.starfield_cubemap` (6 × 1024×1024 / 2048×2048) for offscreen stars when lensing is active, or directly to scene HDR buffer; additive blend, HDR, feeds bloom/diffraction spikes.
  - `common/refraction.glsl` — Shared refraction math: `compute_refraction_angle` (parallax-weighted apparent displacement) vs `compute_refraction_total` (un-parallaxed total ray turn); `compute_refraction_angle` has a dedicated observer-inside closed form for s_min < 0 (object above the camera's horizontal plane): `R(e) = 0.5·delta_cam·erfcx(√(|C|/2H)·sin e)` via `refract_erfcx` (A&S 7.1.26), matching standard refraction tables and staying continuous with the limb model at e = 0; `solve_refraction_apparent` inverts the deflection for point lights/orbits/stars via 12-step bracketed bisection on [0, alpha_0] (the fixed point θ = α(V_app(θ)) is always bracketed there since α is monotone non-increasing); `refract_chord_blocked` tests apparent-ray periapsis to occlude bodies behind the solid planet; `apply_atmospheric_refraction` decouples planetary atmospheric bending from gravitational deflection.
    Also hosts the **unified Gralla–Lupsasca (2020) gravitational lensing** math: `u_grav_lens_*` uniforms (center/rs/radius/type/enabled/strength/spin/pole), `compute_gravitational_deflection` (Gralla–Lupsasca Kerr critical curve $b_c(\phi)$ via `grav_critical_b`, universal Lyapunov exponent $\gamma(a_*, \phi)$, matched 2PN and logarithmic photon-ring divergence, capture → `is_shadow`), and `apply_gravitational_deflection` (+ Lense-Thirring drag), composed into `apply_refraction` / `apply_refraction_eye` for planetary bodies/atmospheres/orbits/point lights, and directly sampled by `grav_lens_starfield.frag` for screen-space unified sky deflection. Validate with `scripts/test_lensing_math.py` and `scripts/test_screen_lensing.py`.
...
- Re-exports post-processing shaders loaded from `engine/glsl/post/`:
  - `grav_lens_starfield.frag` — Screen-space backward gravitational ray deflection pass for the GAIA starfield; uses camera ray basis vectors (`u_cam_forward`, `u_cam_right`, `u_cam_up`, `u_tan_half_fov`) to reconstruct view rays, deflects backward sightlines via `compute_gravitational_deflection` + Kerr frame-dragging. Implements a dual-source sampling architecture: strictly renders on-screen stars at 100% native resolution via `u_starfield_tex` (using integer `texelFetch` for undeflected/unlensed rays and bilinear interpolation for deflected on-screen arcs to eliminate all downsampling/filtering blur), and seamlessly samples `self.starfield_cubemap` via `samplerCube u_starfield_cubemap` for offscreen stars deflected onto the screen from any angle (providing full 4π-steradian sky coverage without boundary clipping), forming continuous Einstein rings, arcs, secondary mirror images, and black hole shadow silhouettes (`gl_FragDepth = 0.999999`).
...
- **`run()`** (~L774) — the whole application lifecycle. Major phases inside:
  ...
  10. **Main render loop** (`while not glfw.window_should_close`): per-frame work incl.
      ...
      - Starfield & Gravitational Lensing pass (~L5160): when lensing is active, performs a dual-source pass: Pass 1a renders unlensed on-screen stars at native screen resolution into `self.starfield_fbo`, Pass 1b renders 360° celestial coverage into the 6 faces of `self.starfield_cubemap` (6 × 1024×1024 / 2048×2048) with 90° FOV perspective projections, and Pass 2 executes screen-space backward deflection (`prog_grav_lens_starfield`) onto `hdr_resolve_fbo` using integer `texelFetch` for 100% pin-sharp unlensed on-screen stars, bilinear texture warping for on-screen Einstein arcs, and seamlessly sampling the cubemap exclusively for offscreen stars deflected onto the screen from any angle; when inactive, renders stars directly to `hdr_resolve_fbo` in a single pass.
...
| Change gravitational lensing / BH shadow | `engine/glsl/common/refraction.glsl`, `engine/glsl/celestial/sphere.*`, `engine/glsl/atmosphere/atmo.*`, `engine/glsl/compute/culling.comp`, `engine/glsl/post/grav_lens_starfield.frag`, `engine/app.py` (lens uniform setup), `engine/physics/physics_core.py` (`load_system_from_data` BH radius) | — |
| Validate lensing math / shadow / offscreen stars | `scripts/test_lensing_math.py`, `scripts/test_screen_lensing.py` | `main` |
