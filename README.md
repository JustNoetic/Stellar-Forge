...
- **Screen-Space Sky Gravitational Lensing (GAIA Starfield)**: Unlike forward point sprites which break into discrete points under strong lensing, the 475k-star GAIA catalog renders once into an offscreen HDR floating-point starfield buffer (`self.starfield_fbo`), which is then processed by a screen-space backward gravitational deflection pass (`post/grav_lens_starfield.frag`). This inverse ray-deflection pass:
  ...
  - Generates continuous, unbroken curved Einstein rings and arcs around the lens without primitive splitting artifacts.
  - Naturally produces secondary mirror images inside $\theta_E$ by sampling the opposite side of the celestial sphere.
  ...
  - Halves catalog draw calls by eliminating the duplicate 460k secondary-catalog draw pass.
