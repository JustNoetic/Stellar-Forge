...
- **🛰️ NASA JPL SPICE & Horizons Integration**: Real-time position playback using official NAIF SPICE kernels (`.bsp`, `.tpc`, `.tls`) with automatic kernel discovery & threaded downloading, plus REST querying of JPL Horizons state vectors. Includes historic spacecraft mission support for **Artemis II** (2026 crewed lunar flyby with adaptive curvature arc-length smoothing) and **Cassini-Huygens** (2004–2017 13-year Saturn tour with dynamic fading trajectory ribbons and dedicated Spacecraft Historic Milestones navigation). Ephemeris systems can be **exported to N-body system JSON** for further simulation.
...
4. **Atmospheric Refraction & Lensing**: Calculates physical ray bending derived from surface refractivity ($n_{\text{mix}} - 1$), scale height, and planetary oblateness. Two complementary quantities are shared via `refraction.glsl`: the parallax-weighted apparent displacement (`compute_refraction_angle`) applied to subpixel point lights, orbits, and habitable-zone visuals, and the total un-parallaxed ray turn (`compute_refraction_total`) used for anchored ray-bending in body meshes (`sphere.frag`), volumetric atmospheres (`atmo.frag`), and rings (`ring.frag`) — keeping both paths consistent at the point↔mesh LOD hand-off. Point lights invert the ray deflection using an unconditionally stable Newton-Raphson solver (`solve_refraction_apparent`) to guarantee monotonic setting trajectories free of inversion singularities even from high orbit.
...
- **Deflection Model** (`common/refraction.glsl`): **Gralla–Lupsasca (2020) analytical Kerr lensing**, featuring the exact closed-form Bardeen (1973) Kerr shadow boundary $b_c(\phi)$, universal fractional Lyapunov exponent $\gamma(a_*, \phi)$, matched 2PN Einstein deflection $\alpha = \frac{2 r_s}{b} + \frac{15\pi}{16}\left(\frac{r_s}{b}\right)^2$, and universal logarithmic photon-ring singularity $\alpha \sim -\frac{1}{\gamma}\ln\left(\frac{b-b_c}{b_c}\right)$ with Lense-Thirring spin frame-dragging.
- **Dual-Source Screen-Space Gravitational Lensing**: Rather than compromising star sharpness or suffering from planar frustum clipping ($z_c \le 0$), background stars from the 475k-star GAIA catalog are rendered using a dual-source pipeline:
  - **On-Screen Stars (`u_starfield_tex`)**: Rendered directly at **100% native screen resolution**. Unlensed stars ($\alpha \le 10^{-7}$) use bit-exact integer `texelFetch` bypassing any filtering blur, while on-screen deflected stars are warped with maximum raster precision.
  - **Offscreen Stars (`u_starfield_cubemap`)**: Rendered into a full 6-face HDR cubemap (6 × 1024×1024 / 2048×2048) providing seamless $4\pi$-steradian omnidirectional sky coverage. Sightlines deflected outside the camera frustum or behind the observer sample from this cubemap, seamlessly pulling offscreen stars from any angle onto the viewport without edge clipping.
  - Generates continuous, unbroken curved Einstein rings and arcs around the lens with native subpixel crispness.
  - Naturally produces secondary mirror images inside $\theta_E$ by sampling opposite 3D directions of the sky cubemap.
  ...
  - When lensing is inactive, the engine automatically bypasses the cubemap and renders background stars directly into the scene HDR buffer in a fast single pass.
- **Exact Bardeen (1973) Black Hole Shadows**: A black hole's mass defines its Schwarzschild radius $r_s = 2GM/c²$. In spinning Kerr spacetime ($a_* \ne 0$), the shadow silhouette boundary $b_c(\phi)$ is computed using the exact Carter constant null geodesic potential $R(r) = r^4 + c_2 r^2 + c_1 r + c_0$. Closed-form cubic stationarity (Viète / Cardano) yields the iconic asymmetric **"D-shape"**: a flat vertical boundary along the prograde edge ($b_{c,\text{pro}} = 1.055\,r_s$ for $a_* = 0.998$), smooth retrograde expansion ($b_{c,\text{ret}} = 3.50\,r_s$), and theoretical polar height $\beta_{\text{max}} = 3\sqrt{3}\,M \approx 2.598\,r_s$. Depth-writing proxy geometry expands automatically to prevent geometric clipping.
...
### NAIF SPICE Kernels
The engine automatically downloads essential kernels (`de440s.bsp`, `pck00010.tpc`, `naif0012.tls`) into `data/kernels/` when switching to Ephemeris Mode via the UI, with progress shown in the **Ephemeris Setup** modal. Additional planetary satellite kernels (e.g. `jup348.bsp`, `sat455.bsp`, `nep104.bsp`, `ura111.bsp`, `plu060.bsp`, `mar099s.bsp`, `codes_300ast_20100725.bsp`) and spacecraft mission kernels can be selected and downloaded directly from the same modal. Ephemeris states can be **exported to an N-Body system JSON** ("Export to N-Body System") for offline integration with the IAS15 or Keplerian engines.

### Historic Spacecraft Missions
- **Artemis II (2026)**: Integrated official SPK binary kernel (`artemis2.bsp`) covering Orion's high-perigee checkout orbit, Trans-Lunar Injection (TLI), and lunar free-return flyby, rendered via adaptive arc-length curvature resampling.
- **Cassini-Huygens (2004–2017)**: Pre-compiled Type 9 SPK binary kernel (`cassini.bsp`, ~3.1 MB, 57,887 states) covering the entire 13.2-year Saturnian orbital tour. Can be generated or refreshed via `scripts/fetch_cassini_kernel.py`. Rendered with a sliding-window $\pm 30$-day cosine-squared alpha fading trail.
- **Spacecraft Historic Milestones Modal**: Accessible in Ephemeris mode via the menu bar (`Tools` or `Physics`) and bottom Time HUD bar (`Milestones`), providing direct epoch jumping, optional auto-tracking, and camera focusing on major milestones (e.g. ICPS separation, Moon closest approach, Saturn Orbit Insertion, Huygens probe release, Enceladus ice plume discoveries, Equinox, and Grand Finale ring dives).
